...
import com.toofan.soft.qsb.api.helper.ApiException
    ...
    suspend fun execute1(
        ...
        if (InternetUtils.isInternetAvailable()) {
            try {
                ...
                val responseCode = connection.responseCode
                if (responseCode !in 200..299) {
                    throw RuntimeException("HttpResponseCode: $responseCode, ${connection.responseMessage}, ${connection.errorStream}")
                } else {
                    val informationString = StringBuilder()
                    BufferedReader(InputStreamReader(connection.inputStream)).use { reader ->
                        var line: String?
